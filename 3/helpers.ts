...
export const getEpsilonRate = (gammaRate:string): string => {
    return gammaRate.split('').map(x => switchBit(x)).join('')
}
...
export const getOxygenGeneratorRating = (data:string[]):string => {
    ...
    while (columnIndex < data[0].length) {
        ...
        if(oxygenGeneratorRating) {
            oxygenGeneratorRating += getMostCommonBit(columnIndex, data.filter(x => x.startsWith(oxygenGeneratorRating)))
        } else {
...
export const getCO2ScrubberRating = (data:string[]):string => {
    ...
    while (columnIndex < data[0].length) {
        ...
        if(cO2ScrubberRating) {
            cO2ScrubberRating += getLeastCommonBit(columnIndex, data.filter(x => x.startsWith(cO2ScrubberRating)))
        } else {
