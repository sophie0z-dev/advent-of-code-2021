...
export const getData = async (fileName?: PathLike): Promise<{ boards: Board[]; calledNumbers: number[] }> => {
  ...
  splitData.slice(1).map((x) => {
    const board: Board = { boardNumbers: [], hasWon: false }
    x.split('\n').map((y) => {
      y.split(' ')
        .filter((value) => value)
        .map((z) => board.boardNumbers.push({ number: parseInt(z), marked: false }))
    })
...
export const checkBoard = (board: Board): boolean => {
  ...
  for (let i = 0; i < 5; i++) {
    const column = [
      ...
      board.boardNumbers[i + 5],
      board.boardNumbers[i + 10],
      board.boardNumbers[i + 15],
      board.boardNumbers[i + 20],
    ]
